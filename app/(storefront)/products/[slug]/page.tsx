...
import { Star, Minus, Plus, ChevronRight, Truck, RefreshCw, Store, MapPin } from "lucide-react";
...
export default function ProductDetailPage({ params }: { params: { slug: string } }) {
  ...
  const sellerStoreName = storeObj?.name || "Official Marketplace Partner";
    ...
    <div className="mx-auto max-w-[1440px] px-4 md:px-16 py-8 md:py-12 w-full pt-[80px] md:pt-[100px] mb-24 md:mb-0">
      ...
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-16">
        ...
        <div className="w-full lg:w-1/2 flex flex-col">
          ...
          {/* Seller Name — Amazon / Flipkart Placement (Below Price) */}
          <div className="flex items-center gap-2 mb-6 text-sm text-foreground-secondary border-b border-border pb-4">
            <Store className="w-4 h-4 text-blue-600" />
            <span>Sold by</span>
            <span className="font-bold text-foreground hover:text-blue-600 transition-colors">
              {sellerStoreName}
            </span>
          </div>
          ...
          <div className="hidden md:block">
            <Button
              variant="primary"
              size="lg"
              className="w-full mb-8 h-14 text-lg font-bold shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all"
            ...
            {[
              ...
              },
              {
                ...
                content: storeObj?.city
                  ? `Shipped directly from ${sellerStoreName} (${storeObj.city}, ${storeObj.state || "India"}).`
                  : `Shipped directly by ${sellerStoreName} via verified courier partners.`,
              },
            ].map((tab) => (
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-background border-t border-border p-4 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] z-40 flex items-center gap-4">
        <div className="flex flex-col">
          ...
          <span className="font-bold text-lg leading-none">{formatCurrency(effectivePrice * quantity)}</span>
        </div>
        <Button variant="primary" className="flex-1 h-12 font-bold shadow-md" onClick={handleAddToCartWithCorrectPrice}>
          Add to Cart
        </Button>
      </div>
    </div>
